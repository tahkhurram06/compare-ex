...
const CHIPSET_INFO = {
  ...
  "Snapdragon 7 Gen 4": {
    score: 60,
    note: "A capable step up from the 7s series, closing some of the gap to flagship silicon",
  },
  "Snapdragon 7s Gen 4": {
    score: 55,
    note: "Solid mid-range performance, a tier below flagship-class chips",
  },
  "Snapdragon 6 Gen 4": {
    score: 45,
    note: "Efficient budget-tier performance built for everyday tasks rather than benchmarks",
  },
  "Dimensity 7300 Pro": {
    score: 48,
    note: "Solid budget-tier performance — capable for everyday use, well behind flagship silicon",
  },
};
...
const BRAND_CAMERA_BONUS = {
  Google: {
    amount: 6,
    reason: "strong computational HDR and low-light processing",
  },
  Apple: {
    amount: 4,
    reason: "accurate color science and class-leading video",
  },
  Samsung: {
    amount: 3,
    reason: "high-resolution sensors and flexible zoom hardware",
  },
};

function parseCameraSpec(cameraStr) {
  const lensCount = cameraStr.split("+").length;
  const mpValues = [...cameraStr.matchAll(/(\d+(?:\.\d+)?)MP/g)].map((m) =>
    parseFloat(m[1]),
  );
  const mainMP = mpValues.length ? Math.max(...mpValues) : 0;
  const zoomValues = [...cameraStr.matchAll(/\((\d+)x\)/g)].map((m) =>
    parseInt(m[1]),
  );
...
export function cameraInsight(phones) {
  ...
  const hardware = reachParts.length
    ? ` thanks to ${reachParts.join(" and ")}`
    : "";
  const brandNote = best.bonus.reason
    ? `, and ${best.phone.brand} phones are known for ${best.bonus.reason}`
    : "";
  const margin = best.score - second.score;
  const closeness =
    margin < 6 ? ` — though ${second.phone.name} isn't far behind` : "";
...
export function displayInsight(phones) {
  ...
  const foldNote = best.isFoldable
    ? " that unfolds into a larger tablet-style screen"
    : "";
  const margin = best.score - second.score;
  const closeness =
    margin < 3 ? ` — a close call against ${second.phone.name}` : "";
...
export function chipsetInsight(phones) {
  ...
  const margin = best.score - second.score;
  const closeness =
    margin < 3 ? ` — very close to ${second.phone.name}'s chip` : "";
